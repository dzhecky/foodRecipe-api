...
const selectAllRecipes = async (paging, keyword, searchBy = 'title', sort = 'updated_time') => {
  return new Promise((resolve, reject) => {
    let query = `SELECT recipes.id_recipe, recipes.title, recipes.ingredients, recipes.photo, recipes.created_time, recipes.updated_time, users.uuid, users.name AS author, users.photo AS photo_author, category.name AS category, category.id_category, (SELECT COUNT(*) AS like FROM event WHERE recipes_id=recipes.id_recipe AND status='like'), (SELECT COUNT(*) AS bookmark FROM event WHERE recipes_id=recipes.id_recipe AND status='bookmark'), (SELECT COUNT(*) AS comments FROM comments WHERE id_recipe=recipes.id_recipe) FROM recipes JOIN users ON recipes.id_user=users.id_user JOIN category ON recipes.id_category=category.id_category WHERE`;
